class LrcSyncApp {
    ...
    initWorkspaceEvents() {
        ...
        window.addEventListener('keydown', (e) => {
            ...
            switch (e.code) {
                case 'Space':
                case 'KeyW':
                case 'KeyS': e.preventDefault(); this.togglePlay(); break;
                case 'KeyA': e.preventDefault(); this.seek(this.centerTime - 1); break;
                case 'KeyD': e.preventDefault(); this.seek(this.centerTime + 1); break;
                case 'KeyQ': e.preventDefault(); this.seekToMark('prev'); break;
                case 'KeyE': e.preventDefault(); this.seekToMark('next'); break;
            }
        });
    }
    ...
    startEditor() {
        ...
        this.lrcData.forEach((item, index) => {
            ...
            // DOM структура узла фразы: бэйдж времени + текст
    ...
    updateLrcDOM() {
        ...
        this.lrcData.forEach(item => {
            const y = halfH + (item.time - this.centerTime) * this.zoom;
            item.el.style.transform = `translateY(${y - 25}px)`; // Сдвиг центра блока
            
            // Обновление метки времени без излишних перерисовок DOM (только при изменении)
